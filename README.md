...
Hermes is a Java 21 / Vert.x WhatsApp chatbot backend. Each organization configures its
...
answers anything off-script. Inbound and outbound WhatsApp are pluggable per org (Twilio or
Meta Cloud API).
...
| [.agents/rules/flow-authoring.md](.agents/rules/flow-authoring.md) | Onboard a new org and author its `flow.json` (every step type, branching, validation) |
